...
методом горячо-холодно рандомно от рандомого числа"""

import numpy as np
def random_predict(n:int, number:int=1) -> int:
    """Угадывает число методом горячо-холодно рандомно от рандомого числа
    ...
    a = np.random.randint(min, max) #предполагаемое число
